...
from app.crud.task import get_tasks, update_task, delete_task
    ...
    try:
        ...
        for task in tasks:
            if task.state == "done" and task.completed_at:
                ...
                logger.debug(f"Checking task {task.id} - completed at: {completed_at.isoformat()}")
                
                if completed_at < cutoff:
                    logger.debug(f"Deleting old completed task: {task.id} - {task.title}")
...
async def process_due_tasks(db: Session) -> None:
    ...
    try:
        ...
        logger.info(f"Starting due task processing for {len(tasks)} tasks. Current time: {now.isoformat()}, Looking up to: {soon.isoformat()}")
        ...
        for task in tasks:
            try:
                if task.state == TaskState.todo and task.due_date:
                    ...
                    logger.debug(f"Checking task {task.id} - due date: {due_date.isoformat()}, state: {task.state}")
                    ...
                    if due_date <= soon:
                        ...
                        logger.debug(f"Updated task state: {task.id} - new state: {updated_task.state}")
                    else:
                        logger.debug(f"Task {task.id} not due yet. Due date: {due_date.isoformat()}")
                else:
                    logger.debug(f"Skipping task {task.id} - state: {task.state}, due date: {task.due_date}")
            except Exception as e:
